...
add_action( 'pmxi_before_post_import', 'wpai_listable_ensure_location_data_is_imported', 10, 1 );
...
function listable_addon_import( $post_id, $data, $import_options, $article ) {
	...
	if ($geo_status == "0" || $geo_status == null) {
	...
	if ( $geocoding_failed ) {
		delete_post_meta( $post_id, "geolocated" );
		$listable_addon->log( "WARNING Geocoding failed with the status: " . $details['status'] );
		if ( array_key_exists( 'error_message', $details ) ) {
			$listable_addon->log( "WARNING Geocoding error message: " . $details['error_message'] );
		}
	}
}
...
function listable_addon_que_scripts( $import_id ) {
    if ( !wp_script_is( 'google-maps' ) ) {
		...
		if ( function_exists( 'pixelgrade_option' ) ) {
			$token = pixelgrade_option( 'mapbox_token', '' );
		} else {
			$token = '';
		}
...
function wpai_listable_ensure_location_data_is_imported ($import_id) {
